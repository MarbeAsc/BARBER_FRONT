...
function EstatusSwitch({
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span
        className={`min-w-12 text-[11px] font-bold tracking-wide ${
    ...
    muiTableBodyRowProps: ({ row }) => ({
      sx: {
        ...
        '&:hover': {
          ...
          boxShadow: '0 8px 18px -16px rgba(79,70,229,.8)',
        },
      },
    }),
      ...
      <div className="border-b border-slate-200 bg-linear-to-r from-slate-50 via-white to-indigo-50/50 px-5 py-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500 sm:text-sm">
              Catálogo base de servicios disponibles para agendar citas.
            </p>
          </div>
        </div>
        {isError ? (
        ...
        <div className="mt-3 rounded-2xl border border-slate-200 bg-white/90 p-3 shadow-sm">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs font-medium">
            <span className="rounded-full bg-blue-50/80 px-2.5 py-1 text-blue-600">Activos: {activeCount}</span>
            <span className="rounded-full bg-indigo-50/80 px-2.5 py-1 text-indigo-600">Inactivos: {inactiveCount}</span>
          </div>
        </div>
      </div>
      <div className="p-4">
